...
public class ChangeRequestService extends CustomBankCheckerManager{
	...
	protected void saveChangeRequest(BankUserContext userContext,ChangeRequest request) throws Exception{
		this.changeRequestManagerOf(userContext).internalSaveChangeRequest(userContext, request);
	}
	
	protected String calcBeanNameFor(BankUserContext userContext,String type) {
	...
	protected ChangeRequestBaseHandler calcHandlerFor(BankUserContext userContext,ChangeRequest request){
		String type = request.getChangeRequestType().getId();
		if(type == null) {
